...
clearBtn.addEventListener("click", () => {
  // console.log(Array.from(searchFilters.children));
  Array.from(searchFilters.children).forEach((filter) => {
    filter.remove();
  });
...
function addFilter(filter) {
  ...
  // button selected
  searchFilters.insertAdjacentHTML(
    "beforeend",
    `
...
function checkFilters(list) {
  return list.map((jobList) => {
    const filters = [
      jobList.role,
      jobList.level,
      ...jobList.languages, // to split item
      ...jobList.tools,
    ];
    ...
    if (selectedFilters.length === 0) {
      ...
      selectedFilters.every(el => {
        console.log(filters.indexOf(el))
      })
                        ...
                        <div class="card__details">
                            <div class="card__company">
                                ...
                                <div class="card__job-types">
                                    ${
                                      filtered.new
                                        ? `<p class="card__job-type card__job-type--new">New!</p>`
                                        : ""
                                    }
                                    ${
                                      filtered.featured
                                        ? `<p class="card__job-type card__job-type--featured">Featured</p>`
                                        : ""
                                    }
                                </div>
                            </div>
                            ...
                            <div class="card__overview">
                                <p class="card__overview-text card__overview-text--postedAt">${
                                  filtered.postedAt
                                }</p>
                                <p class="card__overview-text card__overview-text--contract">${
                                  filtered.contract
                                }</p>
                                <p class="card__overview-text card__overview-text--location">${
                                  filtered.location
                                }</p>
                            </div>
                        </div>
